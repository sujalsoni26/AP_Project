...
//        }
        if (Hero.getWalk() != null){
            Hero.getWalk().pause();
            System.out.println("3. " + Hero.getWalk().getStatus());
        }
        else {
...
//            System.out.println("2 is null");
//        }
        if (Hero.getWalk() != null && Hero.getWalk().getStatus() == Animation.Status.PAUSED){
            Hero.getWalk().play();
            System.out.println("3. " + Hero.getWalk().getStatus());
        }
        else {
...
//            System.out.println("4. " + h1.getCollisionTimer());
    ...
    public void noSavedGame(AnchorPane pane){
        Text noSavedGame = createText("No Saved Game", 277, 259, 26);
        noSavedGame.setFill(Color.WHITE);
        Rectangle rectangle = createRectangle(277, 221, 252, 56, Color.BLACK);
        rectangle.setOpacity(0.35);

        Timeline timeline2 = new Timeline(
                new KeyFrame(Duration.ZERO, event -> {
                    anchorPane.getChildren().addAll(rectangle, noSavedGame);
                }), new KeyFrame(Duration.seconds(3), event -> {
                    anchorPane.getChildren().removeAll(rectangle, noSavedGame);
        }));
        timeline2.setCycleCount(1);
        timeline2.play();
